...
def generate_step_instructions(delta_theta, dps=[9, 9, 9, 9, 9, 9]):
    ...
    dps = np.array(dps)
    print(np.linalg.inv(degrees_per_step.T) @ delta_theta)
    instructions = []
    for _ in range(6):
        ...
        for i in range(6):
            if delta_theta[i] != 0 and dps[i] != 0:
                ...
                dps[i] = 0
        print(degrees_per_step.T[0][0])
        if min_travel_time == np.inf:
            break
    
        steps_per_second = np.linalg.inv(degrees_per_step.T) @ dps
...
animation = \
[[15,15,30,15,15,15],[-15,-15,-30,-15,-15,-15]]
...
inv = np.linalg.inv(degrees_per_step.T)
for row in range(6):
  for col in range(6):
    print(row, col, inv[row][col])